...
class ExperimentHarness:
    ...
    def evaluate_fitness(self, controller_weights: dict, sensor_noise_std: float = 0.0, num_avg: int = 1) -> tuple[float, dict]:
        ...
        with torch.no_grad():
            for t in range(T_total):
                ...
                # This avoids a large, slow pre-computation for high batch sizes.
                ...
                if t >= washout_steps:
                    # Write results directly to pre-allocated GPU tensors
        ...
        # The result tensors are already on the GPU.
        return self._compute_results(results_outputs_A, results_outputs_B, results_settings)
    ...
    def _compute_results(self, outputs_A_gpu: torch.Tensor, outputs_B_gpu: torch.Tensor, settings_log_gpu: torch.Tensor) -> tuple[float, dict]:
        ...
        diagnostics = {
           ...
           "K_effective": sum(p.numel() for p in self.controller.parameters()) / (self.config['substrate_params']['N_A'] + self.config['substrate_params']['N_B']),
        }
